...
Alpha.doCalc = function(phrase, length, allowSpecial, tab, padding, pw, callback) {
    if (length == 0) {
        callback(pw);
        return;
    }
    ...
    setTimeout(function() {
        Alpha.doCalc(phrase, length - 1, allowSpecial, tab, padding, pw, callback);
    }, 1);
}
...
Alpha.calc = function(phrase, username, service, length, allowSpecial, callback) {
    ...
    Alpha.doCalc(phrase, length, allowSpecial, tab, padding, '', callback);
}
