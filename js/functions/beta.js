...
Beta.salt = function(str) {
    var words = CryptoJS.SHA512(str).words;
    for (i = 1; i < words.length; i++)
        words[0] ^= words[i];
    return CryptoJS.SHA512(str + Beta.guids[Math.abs(words[0]) % Beta.guids.length]);
}

Beta.doCalc = function(phrase, length, allowSpecial, salt, tab, padding, pw, callback) {
    if (length == 0) {
        callback(pw);
        return;
    }
    ...
    phrase += CryptoJS.PBKDF2(pw, Beta.salt(pw), { keySize: 512/32 });

    setTimeout(function() {
        Beta.doCalc(phrase, length - 1, allowSpecial, salt, tab, padding, pw, callback);
    }, 1);
}
...
Beta.calc = function(phrase, username, service, length, allowSpecial, callback) {
    ...
    var salt = Beta.salt(phrase + padding);
    Beta.doCalc(phrase, length, allowSpecial, salt, tab, padding, '', callback);
}
